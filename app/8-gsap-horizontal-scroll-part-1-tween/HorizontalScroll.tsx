  ...
  useLayoutEffect(() => {
    ctx = gsap.context((self) => {
      ...
      let scrollTween = gsap.to(sectionsRef.current, {
        ...
        ease: 'none', // Linear easing, i.e., consistent speed.
        scrollTrigger: {
          trigger: sliderRef.current!, // The element that the scroll event is bound to.
          pin: true, // This pins the trigger element in place while the timeline is active.
          scrub: 1, // The animation smoothly follows the scroll position.
          // The end position is dynamically calculated based on the width of the scroller.
          end: () => '+=' + sliderRef.current?.offsetWidth,
        },
      });

      const useScrollTrigger = (triggerEl: any) => ({
        containerAnimation: scrollTween,
        trigger: triggerEl,
        start: 'left center',
        toggleActions: 'play reverse play reverse',
        onEnter: () => {
          const bgColor = triggerEl.closest('.slide').dataset.bgColor;
          gsap.to('.root', {
            background: bgColor,
            duration: 0.5, // adjust this for a smoother transition
          });
        },
        onLeaveBack: () => {
          const sections = sectionsRef.current;
          const idx = sections.indexOf(triggerEl.closest('.slide'));
          if (idx > 0) {
            const prevBgColor = sections[idx - 1].dataset.bgColor;
            gsap.to('.root', {
      ...
      sectionsRef.current.forEach((section) => {
        gsap.from(section.querySelector('h1'), {
          yPercent: 135,
          scrollTrigger: useScrollTrigger(section.querySelector('h1')),
        });
        gsap.from(section.querySelector('p'), {
          yPercent: 50,
          opacity: 0,
          scrollTrigger: useScrollTrigger(section.querySelector('p')),
        });
        // Below the trigger for each element and its animation is the section
        // being the slide itself rather than to ensure the entire section's
        // visibility affects the animation trigger.
        gsap.from(section.querySelector('img'), {
          y: 50,
          opacity: 0,
          scrollTrigger: useScrollTrigger(section),
        });
